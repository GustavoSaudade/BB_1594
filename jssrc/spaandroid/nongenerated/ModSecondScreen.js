...
function loadCategoryProductList() {
    ...
    closeHamburguer2();
    kony.print("@@@@@@@@@@@@@@@@@@@@@@@@@> SEARCHING PRODUCTS FOR CATEGORY: " + currentCategoryId + " - " + currentCategoryName);
...
function processProductListResponse(status, productListResponse) {
    if (status == 400) {
        ...
        if (productListResponse != null && productListResponse.opstatus == 0) {
            kony.print("@@@@@@@@@@@@@@@@@@@@@@@@@> SERVICE RESPONSE SUCESSFUL: " + JSON.stringify(productListResponse));
...
function setPagination(results) {
    kony.print("@@@@@@@@@@@@@@@@@@@@@@@@@> SETTING PAGINATION. CURRENT PAGE = " + results.currentPage + " TOTAL OF PAGES = " + parseInt(results.totalPages));
    ...
    if (numberOfPages >= 1) {
        for (var i = 1; i <= numberOfPages; i++) {
            var listOfPagesSuplement = [i.toString(), i.toString()];
        ...
        FormProductList.LblPagination.text = "Page " + results.currentPage + " of " + results.totalPages;
